...
# Revision 1.1.2.3  1996/12/17  20:49:52  damon
# 	Expanded OSF C O P Y R I G H T   N O T I C E
# 	[1996/12/17  20:47:03  damon]
#
# Revision 1.1.2.2  1996/11/21  16:54:36  weir
# 	Minor corrections
# 	[1996/11/21  16:54:03  weir]
# 
# Revision 1.1.2.1  1996/11/21  16:43:14  weir
# 	Initial submission
# 	[1996/11/21  16:42:15  weir]
# 
# $EndLog$
# 
-->
...
)
{
	...
	{
		fprintf(stdout, "          Setting default route %s ...&bsol;n",
			default_routes[i]);
	...
	if (status != error_status_ok)
	{
		...
		fprintf(stdout, "dce_svc_routing(): %s&bsol;n", error_string);
	/*		exit(1);    */
	}
...
)
{
	...
	if ( (argc == 2) &amp;&amp; (( strcmp(argv[1], "setup") == 0) || &bsol;
	...
	else if (argc &lt; 3)
	{
		...
		fprintf(stdout, "       %s &lt;principal_name&gt; &lt;CDS_dir_name&gt;/&bsol;n&bsol;n", argv[0]);
		exit(1);
	}
	...
	*server_principal_name = (unsigned_char_p_t)malloc(strlen(argv[1]));
	...
	{
		dce_error_inq_text(status, error_string, &amp;print_status);
		fprintf(stdout, "dce_svc_set_progname(): %s&bsol;n", error_string);
		exit(1);
	}
...
)
{
	...
	fprintf(stdout, "Entering create_server_uuid()...&bsol;n");

	/* Create and save server instance UUID...                          */
	fprintf(stdout, "Calling uuid_create()...&bsol;n");
	uuid_create(server_uuid, &amp;status);
	if (status != error_status_ok)
	{
		dce_error_inq_text(status, error_string, &amp;print_status);
		fprintf(stdout, "uuid_create(): %s&bsol;n", error_string);
		exit(1);
	}
	...
	fprintf(stdout, "...Exiting create_server_uuid()&bsol;n");
}
