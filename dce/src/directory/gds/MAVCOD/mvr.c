...
void mvr_arguments(struct asn1_tree *x, int depth, char *indent, FILE *f_out);
void mvr_int_values(struct asn1_tree *x,
					int depth,
					char *indent,
					FILE *f_out);
void look_for_name_up_tree(int x, char *name, FILE *f_out);
...
char *indent;
{
	...
	if (tag_mode != F_UNDEF)
		fputs(implicit_id[tag_mode], f_out);
	fputs("::= BEGIN\n", f_out);
...
mvr_element(x, depth, indent, f_out) struct asn1_tree *x;
int depth;
char *indent;
FILE *f_out;
{
...
AF stattdessen...(Sonderrolle v. ANYDEF) */
			if (tree_element[brother].a_option == O_OPTION && (tree_element[brother].a_type != T_NULL && tree_element[brother].a_type != T_ANYDEF))
			...
			if (tree_element[brother].a_option != O_ALWAYS)
			{
				...
				else
				{
					...
					fprintf(f_out, " = %s)",
							(tree_element[brother].a_default) ? tree_element[brother].a_default : "0");
				}
			}
	...
	case T_CHOICE:
		...
		for (brother = x->a_first_son, i = 1;; i++)
		{
			...
			if (tree_element[brother].a_type != T_NULL)
AF stattdessen...(Sonderrolle v. ANYDEF) */
			if (tree_element[brother].a_type != T_NULL && tree_element[brother].a_type != T_ANYDEF)
...
* 					  *
\*****************************************/
mvr_argument(x, f_out, a_mode) struct asn1_tree *x;
FILE *f_out;
int a_mode;
{
	...
	if (tree_element[i].a_type != T_TYPE)
	{
		...
		default:
			...
			case T_SEQUENCE_OF:
				...
				break;
			default:
				if (x->a_option == O_OPTION && x->a_type != T_ANYDEF /*AF*/
				)
					fputs("[0]", f_out);
				break;
			}
...
mvr_single_argument(x, f_out) struct asn1_tree *x;
FILE *f_out;
{
	putc('(', f_out);
	mvr_argument(x, f_out, A_STD);
...
mvrFLarg(x, f_out, s1) struct asn1_tree *x;
FILE *f_out;
char *s1;
{
	putc('(', f_out);
...
mvr_double_argument(x, f_out, s1, s2) struct asn1_tree *x;
FILE *f_out;
char *s1, *s2;
{
	putc('(', f_out);
...
* 				        *
\***************************************/
mvr_allocate(el, depth, indent, f_out, a_mode) int el;
int depth;
char *indent;
FILE *f_out;
int a_mode;
{
	...
	case T_COPY: /*AF COPY ergaenzt*/
		...
		*/
		break;
	default:
		fputs(tree_element[el].a_name, f_out);
		break;
	}
	putc(')', f_out);
}
...
look_for_name_at_level(father, name, f_out, tried) int father;
char *name;
FILE *f_out;
int tried;
{
	...
	{
		if (tree_element[i].a_id != 0 && strcmp(name, tree_element[i].a_id) == 0)
		{
