...
char *build_element_name(element) int element;
{
	char *x, *new_name;
...
promote_structure(element) int element;
{
	char *new_name;
...
void look_for_optional(int element, int father_needs_pointer)
{
	int i;
...
sort_data_types(list, nb_in_list) int *list, nb_in_list;
{
...
found_reference(el, list, nb_in_list) int el, *list, nb_in_list;
{
	int i;
	...
	{
		if ((tree_element[el].a_type == T_EXTERN || tree_element[el].a_type == T_COMP) && (i = found_in_list(tree_element[el].a_name, list, nb_in_list)) != -1 && !(data_is_structured(list[i]) && type_need_pointer(el)))
		{
			return (i);
		}
...
found_in_list(s, list, nb_in_list) char *s;
int *list, nb_in_list;
{
	int i;
...
build_ordered_list(list) int **list;
{
	int nb_in_list = 0, i;
		 ...
		 i = tree_element[i].a_brother)
	{
		if ((nb_in_list & 0x7F) == 0)
			*list = (int *)((nb_in_list) ? realloc((char *)*list, (unsigned)(128 + i) * sizeof(int)) :
										 /*AF*/
