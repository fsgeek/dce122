...
void stxerr(char *s1, char *s2);
void one_more_include(char *buf);
void verify_comp(struct comp *c, int ft, int up);
...
void defined_procedure(proc_name) char *proc_name;
{
	int i;
...
* 									   *
\**************************************************************************/
opencomp(id, n, m, i, t, ut, o, s1, s2, cm) int n, m, i, t, o, cm, ut;
char *s1, *s2, *id;
{
	...
	c = complist + compindex;
	c->c_line_num = linenum;
	c->c_number = n;
	c->c_mode = m;
	c->c_implicit = i;
	c->c_type = t;
...
* 					   *
\******************************************/
closecomp(id, n, m, i, t, ut, o, s1, s2, cm) int n, m, i, t, o, cm, ut;
char *s1, *s2, *id;
{
	register struct comp *c;
...
void one_more_include(char *buf)
{
	...
	if (incl_list_nb == 0)
		...
		incl_list = (char **)realloc((char *)incl_list,
...
verify_tree()
{
	...
	for (i_comp = 0; i_comp < compindex; i_comp++)
	{
		...
		{
			if (complist[i_comp].c_type != T_TYPE && complist[i_comp].c_type != T_IMPORT)
			{
	...
	for (i_comp = compindex - 1; i_comp >= 0; i_comp--)
	{
		...
		for (i_first = complist[i_comp].c_first_son;
			...
			if (complist[i_first].c_type != T_PARAMETER)
				break;
		...
		default:
			complist[i_comp].c_call_mode = COMPLEX_MODE;
			if (nb_exported == -1 || complist[i_first].c_implicit == 0 || complist[i_first].c_mode != -1)
			{
				break;
			}
			if (nb_exported)
			...
			if (complist[i_comp].c_call_mode != COMPLEX_MODE && complist[i_comp].c_type == T_EXTERN)
			{
				for (k = i_comp + 1; k < compindex; k++)
				{
					if (complist[k].c_type != T_TYPE &&
						...
						continue;
					if (complist[k].c_call_mode == DEFINE_MODE || complist[k].c_call_mode == HALF_MODE)
						complist[i_comp].c_call_mode =
							COMPLEX_MODE;
					else
						k = compindex;
...
void verify_comp(struct comp *c, int ft, int up)
{
	...
	if (c->c_type == T_DEFAULT && (c->c_father == -1 || ((i = complist[c->c_father].c_father) == -1) ||
								   (complist[i].c_type != T_SET && complist[i].c_type != T_SEQUENCE)))
	{
		i = c->c_father;
		stxerr("DEFAULT is only allowed within SET or SEQUENCE",
			   (i == -1) ? (char *)0 : ((complist[i].c_id) ? complist[i].c_id : complist[i].c_name));
	}
	...
	if (c->c_type != 0 && c->c_type != T_PARAMETER)
	{
		...
		if (c->c_type == T_COMP && complist[c->c_father].c_type != T_SET && complist[c->c_father].c_type != T_SEQUENCE)
			stxerr(
		...
		if (c->c_type == T_CHOICE || c->c_type == T_SET)
			...
			{
				if (complist[i].c_type == T_INIT || complist[i].c_type == T_FINAL || complist[i].c_type == T_DEFAULT || complist[i].c_type == T_ALLOC || complist[i].c_type == T_ARGUMENT)
					continue;
				...
				if (complist[i].c_type == T_EXTERN && complist[i].c_mode == -1)
				{
				...
				if (complist[i].c_type == T_CHOICE && complist[i].c_mode == -1)
					...
					match_in_list(complist[i].c_mode,
...
match_in_tree(i0, i1)
{
	...
	for (i = i0; i != -1; i = complist[i].c_brother)
	{
		...
		if (complist[i].c_type == T_CHOICE && complist[i].c_mode == -1)
			...
			match_in_list(complist[i].c_mode, complist[i].c_number, i1);
	}
}
...
match_in_list(m, n, j0)
{
	...
	for (j = j0; j != -1; j = complist[j].c_brother)
	{
		if (complist[j].c_type == T_INIT ||
			...
			complist[j].c_type == T_COMP)
			continue;
		if (complist[j].c_type == T_EXTERN && complist[j].c_mode == -1)
			continue;
		if (complist[j].c_type == T_CHOICE && complist[j].c_mode == -1)
		{
			match_in_list(m, n, complist[j].c_first_son);
			continue;
		}
		if (complist[j].c_mode == m && complist[j].c_number == n)
		{
...
sanitize_quoted_string(result, original) char *result, *original;
{
	register char *x, *y;
