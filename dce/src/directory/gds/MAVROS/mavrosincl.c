...
void incltype(struct comp *c);
void decl_routine(struct comp *c, int cmode);
void macrogen(struct comp *foo, struct comp *bar, int callmode);
...
int generincl(void)
{
	...
	{
		if (complist[i].c_type != T_VALUE || complist[complist[i].c_father].c_type == T_FLAGS)
		{
			if (k == 1)
				k = 2;
			continue;
	...
	for (i = 0, k = 0; i < compindex; i++)
	{
		if (complist[i].c_type != T_TYPE || (complist[i].c_call_mode != DEFINE_MODE && complist[i].c_call_mode != HALF_MODE))
			continue;
		if (!k)
		...
		for (j = complist[i].c_first_son; j != -1;
			...
			if (complist[j].c_type != T_PARAMETER)
				break;
	...
	for (i = 0, k = 0; i < compindex; i++)
	{
		if (complist[i].c_type != T_TYPE || complist[i].c_call_mode == DEFINE_MODE)
			continue;
		if (!k)
...
void decl_routine(struct comp *c, int cmode)
{
	...
	for (j = 0; j < 2; j++)
	{
		for (i = c->c_first_son; i != -1; i = complist[i].c_brother)
		{
			...
			if (callmode == 0 && complist[i].c_call_mode == 1 || callmode == 1 && complist[i].c_call_mode != 2)
				(void)printf("*");
		}
		if (cmode != C_CPY)
			break;
...
void macrogen(struct comp *foo, struct comp *bar, int callmode)
{
	...
	title = (bar->c_type == T_EXTERN) ? bar->c_name : extern_title(bar->c_type);
	...
	if (bar->c_type == T_EXTERN)
		...
		(void)cdbasic(bar, 0, 0, -1, extcall, 0);
	(void)printf(")\n");
	return;
}

char *extern_title(i) int i;
{
	int j, allocate = 0;
...
inclmatch(c) struct comp *c;
{
	...
	for (first_comp = c->c_first_son; first_comp != -1;
		...
		if (complist[first_comp].c_type != T_PARAMETER)
			break;
	if (complist[first_comp].c_mode == -1 && (complist[first_comp].c_type == T_ANY || complist[first_comp].c_type == T_ANY_ALLOC || complist[first_comp].c_type == T_ANYDEF))
		(void)printf("1");
	else if (complist[first_comp].c_type == T_CHOICE && complist[first_comp].c_mode == -1)
	{
		match_incl_choice(&complist[first_comp]);
	}
	else if (complist[first_comp].c_type == T_EXTERN && complist[first_comp].c_mode == -1)
	{
		match_incl_extern(&complist[first_comp]);
	}
	else
		match_incl_asn1(complist[first_comp].c_mode,
						complist[first_comp].c_number, asnstruct(&complist[first_comp]));
	(void)printf(")\n");
}

match_incl_extern(c) struct comp *c;
{
	(void)printf("%s%s(z)", c->c_name, exec_table[MATCH].suffix);
}

match_incl_choice(c) struct comp *c;
{
	int i, first = 0;
	...
	for (i = c->c_first_son; i != -1; i = complist[i].c_brother)
	{
		...
		if (first++)
			(void)printf("\\\n||\t");
		if (complist[i].c_mode == -1 && complist[i].c_type == T_CHOICE)
			match_incl_choice(&complist[i]);
		else if (complist[i].c_mode == -1 && complist[i].c_type == T_EXTERN)
			match_incl_extern(&complist[i]);
		else if (complist[i].c_mode == -1 && (complist[i].c_type == T_ANY || complist[i].c_type == T_ANY_ALLOC || complist[i].c_type == T_ANYDEF))
			(void)printf("(1)");
		else
		{
			(void)printf("(");
...
match_incl_asn1(m, n, t) int m, n, t;
{
	unsigned char x;
	int l = 0, k, i = 0;
