...
void tree_print(void)
{
	...
	{
		if (complist[i].c_type == T_IMPORT && complist[i].c_index != 0)
		{
			last = i + 1;
		}
	}
...
{
	int flag = 0;
	if (i != -1 && complist[i].c_type == T_PARAMETER && complist[i].c_brother != -1)
	{
...
void printcomp(int i, int j)
{
	...
	if (complist[i].c_type != T_SET_OF && complist[i].c_type != T_SEQUENCE_OF)
	{
		mode_print(i);
	}
	switch (complist[i].c_type)
	...
	case T_CHOICE:
		...
		{
			if (complist[k].c_type == T_ALLOC || complist[k].c_type == T_DEFAULT || complist[k].c_type == T_FINAL || complist[k].c_type == T_INIT)
				break;
			for (l = 0; l < j; l++)
				(void)printf("\t");
	...
	default:
		...
		{
			if (complist[k].c_type == T_ALLOC || complist[k].c_type == T_DEFAULT || complist[k].c_type == T_INIT)
				break;
			if (k != k0)
			{
				(void)printf(",\n");
	...
	while (k != -1)
	{
		...
		else if (complist[k].c_type == T_INIT || complist[k].c_type == T_FINAL || complist[k].c_type == T_DEFAULT)
		{
	...
	if (complist[i].c_optional && !defaulted)
		(void)printf(" OPTIONAL");
	return;
}

quoted_string_print(s) char *s;
{
	register x;
...
mode_print(i) int i;
{
	...
	{
		if (complist[i].c_id)
			(void)printf("%s", complist[i].c_id);
		switch (complist[i].c_mode)
		{
		case 0:
			(void)printf("[UNIVERSAL %d] ", complist[i].c_number);
			break;
		case 1:
			(void)printf("[APPLICATION %d] ", complist[i].c_number);
			break;
		case 2:
			(void)printf("[%d] ", complist[i].c_number);
			break;
		case 3:
			(void)printf("[PRIVATE %d] ", complist[i].c_number);
			break;
		}
		if (complist[i].c_implicit)
