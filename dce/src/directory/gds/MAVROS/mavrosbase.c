...
void store_in_list(char *s)
{
	int i;

	for (i = 0; i < storeindex; i++)
		if (strcmp(s, storelist[i]) == 0)
			return;
...
get_depth(c, d, sc) struct comp *c;
int d, *sc;
{
	...
	*sc = 0;
	...
	if (c->c_type == T_PARAMETER)
		return (d);
	d += (c->c_mode != -1 && c->c_implicit == 0) ? 2 : 1;
	if (c->c_type == T_COMP || c->c_type == T_EXTERN)
		return (d);

	for (i = c->c_first_son; i != -1; i = complist[i].c_brother)
	{
		if (complist[i].c_type == T_INIT || complist[i].c_type == T_FINAL || complist[i].c_type == T_DEFAULT || complist[i].c_type == T_ALLOC)
			break;
		if ((j = get_depth(&complist[i], 0, &setc)) > k)
			k = j;
		if (setc > mset)
			mset = setc;
		comp++;
...
arglst(c_first_son, l0, nocomma) int c_first_son, l0, nocomma;
{
	int i;

	for (i = c_first_son; i != -1; i = complist[i].c_brother)
	{
		if (complist[i].c_type != T_PARAMETER)
			break;
		if (nocomma || i != c_first_son)
			(void)printf(",");
...
arglst_cpy(c_first_son, l0, nocomma) int c_first_son, l0, nocomma;
{
	int i;
...
argdcl(c_first_son, m, l0) int c_first_son, m, l0;
{
	int i;
	...
	for (i = c_first_son; i != -1; i = complist[i].c_brother)
	{
		...
		if ((m == 0 && complist[i].c_call_mode == 1) || (m == 1 && complist[i].c_call_mode != 2))
			(void)printf(" *");
		(void)printf(" %s;\n", complist[i].c_name);
		l0++;
	}
...
argdcl_cpy(c_first_son, m, l0) int c_first_son, m, l0;
{
	int i;
...
int m, l0;
{
	...
	if (c->c_type == T_ARGUMENT)
	{
		...
		{
			if (complist[k].c_type != T_PARAMETER)
				break;
			if (strcmp(complist[k].c_name, c->c_name))
				continue;
	...
	else
	{
		...
		for (i = c->c_first_son; i != -1; i = complist[i].c_brother)
		{
			if (complist[i].c_type == T_ARGUMENT)
			{
				...
				(void)printf("%s%s", (complist[i].c_call_mode) ? "->" : ".",
							 complist[i].c_name);
		}
	}
	return (l0);
...
cpy_argprint(c, m, l0) struct comp *c;
int m, l0;
{
	int i, k, local_mode = 0;
	char *found_in_list = "";
...
void cd_extern_or_basic_call(int callmode, int m, int n, char *element_name)
{
	...
	case ENCODE:
		if (m != -1 && (m & HEAD))
			...
			(void)printf("asn1z,%d,%d|asn1_x509", m, n);
		break;
	case LENGTH:
		(void)printf("%d", n);
		break;
...
index_basic_op(i, allocate) int i, *allocate;
{
	switch (i)
	{
	case T_NULL:
...
print_basic_args(c, op_index, j, notfirstarg, arg_mode, l0) struct comp *c;
int op_index;
int j, notfirstarg, arg_mode, l0;
{
	int i, dist_mode;

	for (i = 0; i < basic_table[op_index].bt_nb_parms && j != -1 && complist[j].c_type == T_ARGUMENT;
...
cdbasic(c, m, n, l0, callmode, allocmode) struct comp *c;
int m, n, l0, callmode;
{
	int i, j, op_index;
	...
	{
		cd_extern_or_basic_call(callmode, m, n,
								basic_table[op_index].bt_name);
	}
...
* 									     *
\****************************************************************************/
which_extern_type(name) char *name;
{
	int j;

	for (j = 0; j < compindex; j++)
		if ((complist[j].c_type == T_TYPE || complist[j].c_type == T_IMPORT) && strcmp(complist[j].c_name, name) == 0)
			return (j);
	return (-1);
}

print_extern_args(c, cref, l0, notfirstarg, arg_mode) struct comp *c, *cref;
...
cdextern(c, m, n, l0, callmode) struct comp *c;
int m, n, l0, callmode;
{
	int i, j;
	int dist_mode = 0, notfirstarg, arg_mode, not_including;
...
* 								        *
\***********************************************************************/
cd_anydef(c, l0, cmode) struct comp *c;
int l0;
int cmode;
{
	...
	for (i = complist[the_arg].c_brother; i != -1 && complist[i].c_type == T_ARGUMENT;
	...
	if (cmode == C_CPY)
	{
		...
		if (af_anydef)
			...
			l0 = cpy_argprint(&complist[the_arg], 1, l0);
	}
	(void)printf(");\n");
	l0++;
	return (l0);
...
* 								        *
\***********************************************************************/
decl_proc_head(c, l0, cmode) struct comp *c;
int l0;
int cmode;
{
	int callmode;
...
proc_headline(c, l0, cmode) struct comp *c;
int l0, cmode;
{
...
#endif
		l0++;
		if (found == 0 && strncmp(incl_list[i], package, lp) == 0 && strcmp(incl_list[i] + lp, ".h") == 0)
			found = 1;
	}
	if (found == 0)
	{
...
genermake()
{
	...
	if (compilation_mode & ONE_FILE)
	{
		...
		for (i = 0; i < compindex; i++)
		{
			...
			{
			case T_TYPE:
				if (complist[i].c_optional != 0 || complist[i].c_call_mode == DEFINE_MODE)
					break;
				(void)printf("\\\n\t");
...
open_module_file(type_name, mode, l0) char *type_name;
int mode, l0;
{
	char fname[256];
...
is_head_of_file(c_name, c_mode, type_mode) char *c_name;
int c_mode, type_mode;
{
	if (is_declared_procedure(c_name, c_mode))
	{
		if (!(c_mode == DECODE && is_declared_procedure(c_name, FREE) == 0 && is_declared_procedure(c_name, ERRCOD) == 0))
			return (compilation_mode & ASN1CODER);
		else if (c_mode == ENCODE && is_declared_procedure(c_name, LENGTH) == 0)
...
is_declared_procedure(c_name, exec_mode) char *c_name;
int exec_mode;
{
	int i;
	char procname[256];
