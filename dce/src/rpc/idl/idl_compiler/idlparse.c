...
typedef struct
{
    ...
    int                 *yyps;
#else
    // This seems to be the equivalent data for the current template, but it
    // stores more than just the current top of the value and state stacks
    // see: https://www.mkssoftware.com/docs/man5/mks_yaccplus.5.asp
    YYSTACKDATA yystackdata;
#endif // 0
    YACC_INT yystate;
    int yytmp;
    int yynerrs;
...
lex_yacc_state_t NIDL_save_lex_yacc_state()
{
    ...
    save_state->yytmp = yytmp; /* maybe yydebug? */
    save_state->yypvt = yypvt; /* seem to be the top value of the stack */
#else
    save_state->yystackdata = yystack;
#endif // 0
    ...
    /* save_state->yymorfg = yymorfg; <-- not part of current template */
...
#if 0
    /* it seems the current template uses yystack for a variable sized stack, so saving that
     * (above) should be enough to preserve the needed state.
     */
    ...
    lex_yacc_state_t state_ptr;
{
...
#else
    /* This should be enough using the current (2017) template */
    yystack = saved_state->yystackdata;
#endif // 0
    ...
    /* yymorfg = saved_state->yymorfg; <--- not part of current template */
...
#if 0
    /* Just like we no longer save all the individual variables, we no
     * longer need to save them either
     */
