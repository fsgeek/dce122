 ...
 * HISTORY
 * $Log: error.c,v $
 * Revision 1.1.8.3  1994/01/25  22:39:20  marty
 * 	CR # 1013 - Fix sinix bsubmit/merge bug in err_log().
 * 	[1994/01/25  22:39:09  marty]
 *
 * Revision 1.1.8.2  1994/01/24  20:05:18  marty
 * 	CR # 1002 - Change the error message given when bcreate -undo has wrong revision branch.
...
struct
{
  ...
  int severity;
    ...
    {"Cannot remove file while it still has revision %s -- try bcs -o first", FATAL},
...
void err_append(ERR_LOG log, ERR_LOG log_append)
{
  ERR_LOG log_ptr;
