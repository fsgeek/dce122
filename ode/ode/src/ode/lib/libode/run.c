 ...
 * HISTORY
 * $Log: run.c,v $
 * Revision 1.8.13.2  1994/02/18  21:44:57  damon
 * 	CR 1056. added const to char * for run()
 * 	[1994/02/18  21:44:51  damon]
 *
 * Revision 1.8.13.1  1994/01/21  20:29:16  marty
 * 	CR # 865 - Can only have one COPY RIGHT NOTICE marker in file
 * 	[1994/01/21  20:27:31  marty]
 *
...
int runvp(const char *name, char *const *argv)
{
	return (dorun(name, argv, 1));
}

static int dorun(const char *name, char *const *argv, int usepath)
{
	int wpid;
	register int pid;
	struct sigaction ignoresig, intsig, quitsig;
	int status;
	int (*execrtn)(const char *, char *const dargv[]) = usepath ? execvp : execv;
