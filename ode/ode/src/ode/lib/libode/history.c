 ...
 * HISTORY
 * $Log: history.c,v $
 * Revision 1.1.11.7  1994/02/22  16:04:36  damon
 * 	CR 1049. Pedantic changes
 * 	[1994/02/22  16:04:19  damon]
 *
 * Revision 1.1.11.6  1994/02/08  19:46:46  marty
 * 	CR # 917 - Remove updating of RCSfile string.
 * 	[1994/02/08  19:32:50  marty]
 *
 * Revision 1.1.11.5  1994/01/20  22:12:45  marty
 * 	CR # 917 - More fixes
 * 	[1994/01/20  22:11:04  marty]
 *
 * 	CR # 917 - Adjust for RCSfile lines
 * 	[1994/01/20  21:08:12  marty]
 *
 * Revision 1.1.11.4  1994/01/20  00:15:33  marty
 * 	CR # 917 - Leave file in sandbox identical to checked in version.
 * 	[1994/01/20  00:11:53  marty]
 *
 * Revision 1.1.11.3  1994/01/12  16:28:53  marty
 * 	CR # 917 - Final touch up so that file left in sandbox after 'bci' is
 * 	the same as what's in RCS.
 * 	[1994/01/12  16:28:26  marty]
 *
 * Revision 1.1.11.2  1993/12/28  22:38:08  damon
 * 	CR 951. create_leaderless_log() now works with 1 or 1+ branches
 * 	[1993/12/28  22:36:34  damon]
 *
 * Revision 1.1.11.1  1993/12/08  22:12:03  marty
 * 	 CR # 840 - Place temporary files in BCSTEMP
 * 	[1993/12/08  22:09:54  marty]
 *
 ...
 * Prototypes
 */
void hst_add_comment(HIST_ELEM node, char *text);
void hst_mark_common_entries(HIST_ELEM ver_ancestor, HIST_ELEM check);
void hst_dump_list_fd(HIST_ELEM history, FILE *fd);
struct comment_elem *
...
STATIC char *getline_local(char *buf, int bufsiz, FILE *fp, int sameline, ERR_LOG *log);
...
src_ctl_extract_history(FILE *mesgf, char *leader, char **branches, char *rev)
{
  ...
  if (log != OE_OK)
    return (log);
  if (!seenmarker ||
      (havestamp && (log = savelog(&loghead, &logtail, &logtemp)) != OE_OK) ||
...
STATIC
char *
getline_local(char *buf, int bufsiz, FILE *fp, int sameline, ERR_LOG *log)
{
  if (!sameline)
  {
...
STATIC
int search_for(
    char *buf,          /* buffer for getline_local */
    ...
    ERR_LOG *log)
{
  ...
  *good_leader = FALSE;
  ...
  case 1:
    ...
    */
    while ((res = getline_local(buf, size, inf, reuse, log)) != NULL)
    {
      reuse = FALSE;
  ...
  case 2:
    ...
    */
  case 3:
    if ((res = getline_local(buf, size, inf, reuse, log)) != NULL)
    {
      reuse = FALSE;
      *good_leader = (strncmp(buf, leader, lead_len) == 0);
      if ((*ptr = strstr(buf, search)) != NULL)
        ...
        */
        for (;;)
        {
          if (oxm_gets(rcs_monitor, buffer, sizeof(buffer), &log) != NULL)
          {
            ...
            if (strncmp(buffer, "*** Initial Branch Revision ***", 31) == 0)
            {
              break;
            } /* if */
          }
...
history = hst_alloc_entry (comment_leader, logmsg, revision, ch_date, ch_time);
...
hst_xtract_file(char *fpath, char *comment_leader)
{
  ...
  concat(buf, sizeof(buf), fpath, ".hist.tmp", NULL);
  strcpy(tmp_path, tmpfile_name(buf));
...
void hst_insert_file(char *fpath, HIST_ELEM history, char *comment_leader, char *RCSfilerev,
                     char RCSfiledate[], char RCSfiletime[])
{
  ...
  concat(buf_path, sizeof(buf_path), fpath, ".hist.tmp", NULL);
  strcpy(tmp_path, tmpfile_name(buf_path));
...
HIST_ELEM
hst_alloc_entry(char *comment_leader, char *logmsg, char *revision, char ch_date[],
                char ch_time[])
{
  ...
  concat(line, sizeof(line), comment_leader, "Revision ", node->header.revision,
         "  ", ch_date, "  ", ch_time, "  ", node->header.user, "\n", NULL);
  hst_add_comment(node, line);
  concat(comment, sizeof(comment), comment_leader, logmsg, NULL);
  hst_add_comment(node, comment);

  /* log message */
  strcpy(ldr, comment_leader);
  i = strlen(comment_leader);
  if (i > 0)
  {
    if (ldr[i - 1] == ' ')
      ldr[i - 1] = '\0';
  }
  concat(comment, sizeof(comment), ldr, "\n", NULL);
  hst_add_comment(node, comment);
...
void hst_dump_list(HIST_ELEM history)
{
  struct hist_elem *ptr;
  struct comment_elem *cmts;
...
void gm_get_date(char *date,  /* date in mm/dd/yy */
...

{
  ...
  t_clock = time((long *)0);
  time_info = gmtime(&t_clock);
  /* FIX ME ... Hard coded century shouldn't be here. */
  sprintf(date, "19%d/%02d/%02d", time_info->tm_year,
          time_info->tm_mon + 1, time_info->tm_mday);

  sprintf(ltime, "%02d:%02d:%02d", time_info->tm_hour, time_info->tm_min,
          time_info->tm_sec);
}

void hst_freelist(HIST_ELEM node)
{
