 ...
 * HISTORY
 * $Log: interface.c,v $
 * Revision 1.3.15.3  1994/03/01  21:00:55  damon
 * 	CR 1063. Check optarg result for (char)0 as well as NULL
 * 	[1994/03/01  21:00:14  damon]
 *
 * Revision 1.3.15.2  1993/12/30  15:54:32  damon
 * 	CR 952. Fixed CONT which was broken by last submit
 * 	[1993/12/30  15:51:37  damon]
 *
 * Revision 1.3.15.1  1993/12/29  18:00:11  damon
 * 	CR 952. Added VERROR, check for no sb
 * 	[1993/12/29  17:59:00  damon]
 *
...
void ui_set_progname(const char *newname)
...

{
  ...
  if (NULL == outfile[0])
  {
    ...
    outfile[index++] = stdout; /* VALWAYS */
    outfile[index++] = stderr; /* VERROR */
    outfile[index++] = stderr; /* VFATAL */
    outfile[index++] = stdout; /* VQUIET */
    outfile[index++] = stderr; /* VWARN */
    outfile[index++] = stdout; /* VNORMAL */
    outfile[index++] = stderr; /* VDIAG */
    outfile[index++] = stdout; /* VDETAIL */
    outfile[index++] = stderr; /* VDEBUG */
  }
      ...
      (msg_verbosity == VCONT && last_ver <= verbosity))
  {
    ...
    if (msg_verbosity == VFATAL || msg_verbosity == VERROR || msg_verbosity == VWARN || msg_verbosity == VDIAG || msg_verbosity == VDEBUG)
    ...
    {
      if (last_ver == VFATAL || last_ver == VERROR || last_ver == VWARN || last_ver == VDIAG || last_ver == VDEBUG)
  ...
  *rcargc = 0; /* Allocate 10 args, realloc later if more needed. */
  ...
  for (;;)
  {
    ...
    if (*rcargc >= alloc_arg)
    {
      ...
      if ((*rcargv = (const char **)realloc(*rcargv, sizeof(char **) * alloc_arg)) == NULL)
        return (FALSE);
    } /* if */
...

{
  ...
  while (ct < usrct)
  {
    ...
    {
      if (special_case(opt))
      { /* try built in, special case before user */
        ct++;
        continue;
      } /* if */
...

{
  ...
  while (*argct < hptr->min_args)
  { /* get min number fields */
    if (*pcount + 1 < usrct)
      (*pcount)++;
...

{
  ...
  {
    if (special_case(pattern))
    { /* try built in, special case */
      va_end(arglist);
      from_ui_load = FALSE;
      return (rvalue);
    } /* if */
  ...
  if (rvalue != ERROR)
  {
    if (argct > 0)
    {
      ...
      if ((vlist = (const char **)calloc(argct, sizeof(char *))) == NULL)
...

{
  ...
  {
  case VQUIET:
    return (strdup(QUIET_OP));
  case VNORMAL:
    return (strdup(NORMAL_OP));
  case VDETAIL:
    return (strdup(VERBOSE_OP));
  case VDEBUG:
    return (strdup(DEBUG_OP));
  default:
    ui_print(VFATAL, "illegal level of verbosity: %d.\n",
             verbosity);
    return (NULL);
  } /* switch */
} /* ui verbose switch */
