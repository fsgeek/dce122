...
int fd_runcmdv(const char *cmd, const char *dir, int withpath, int closefd,
               int outfd, char **av)
{
    int child;
    int fd;
...
int runcmdv(const char *cmd, const char *dir, char **av)
{
...
#else
    runcmd(cmd, dir, va_alist) char *cmd,
    *dir;
va_dcl
#endif
{
