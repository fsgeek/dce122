...
#define SETGID 02000
#define STICKY 01000
/* end of defines for getmode_local() */
int usflag;
...
void verify(char *link_from, char *link_to);
void create_files(struct dir_entry *node, const char *reldir);
void update_tree(struct dir_entry *node, const char *base, const char *reldir);
void get_lists(struct dir_entry *node, const char *base, const char *rel_path);
...
/* Builds up list of all files in
					   backing chain.                     */
int symlink_file(char *, char *); /* Create symbolic link.              */
int copy_file(char *, char *);	  /* Copy file to sandbox.              */
int cmp_files(char *, char *);	  /* Compare files 	              */
void get_current_directory(char *, char[]);
...
int file_type(const char *base, const char *reldir, const char *file)
{
...
int search_sandbox(const char *reldir, const char *file)
{
	char file_path[PATH_LEN];
...
void create_file(char *file_to_create, char *file_from_backing_tree)
{
...
void get_current_directory(char *curdir, char sandboxdir[])
{
 ...
 *           argument to mkdir().  getmode_local has to know the default
