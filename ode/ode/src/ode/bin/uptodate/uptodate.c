...
static void update_update(FILEREV *head, /* head of list of file/revs */
                          char *sbbase   /* holds dir to sb base */
);

static void do_copy(
    const char *src, /* where the file will be copied from */
...

{
  ...
  while (*from != NUL)
  { /* extract problems from each string */
    if ((*to++ = *from++) != SLASH)
      continue;
