...
static char *rcsid =
	"$RCSfile: release.c,v $ $Revision: 1.12.10.1 $ $Date: 1993/12/09 19:02:19 $";
...
/* compilers are fussier now than they used to be */
void do_release(void);
void get_component_info(void);
...
parse_command_line(argc, argv) int argc;
char **argv;
{
	int i;
...
#ifndef _BLD
check_sandbox(rcfile_p) struct rcfile *rcfile_p;
{
...
parse_stages(args_p) struct arg_list *args_p;
{
...
find_stage(stage, stage_pp) char *stage;
struct stage **stage_pp;
{
	struct stage *stage_p;
	char **pp;
...
get_release_id(type, value) char *type, *value;
{
	FILE *fp;
...
compare_component_info(file_ptr, file_info_ptr) struct file *file_ptr;
...
check_release_stage(stage_p) struct stage *stage_p;
{
...
checkstate(file, statp) char *file;
struct stat *statp;
{
	int errs = 0;
...
get_old_log(stage_p, fd_p, temp) struct stage *stage_p;
int *fd_p;
char *temp;
{
	char buf[MAXPATHLEN];
...
#else  /* _BLD */
do_stage_release(fstage_p, tstage_p) struct stage *fstage_p, *tstage_p;
{
	struct stat statb;
	char *ptr;
...
ckmakepath(pathname, refbase, refpath) char *pathname, *refbase, *refpath;
{
...
try_to_open(file, how, mode) char *file;
int how, mode;
{
	int fd;
	int status;
...
#ifndef _BLD
keep_lines(filename, fd, kept) char *filename;
int fd;
int *kept;
{
	FILE *fp;
...
flush_lines(filename, fd) char *filename;
int fd;
{
	FILE *fp;
	char buf[BUFSIZ];
...
create_old_log_db(fromlogtmp) char *fromlogtmp;
{
	int fd;
	int status;
...
create_new_log_db(tologtmp) char *tologtmp;
{
	FILE *fp;
	char buf[BUFSIZ];
	int status;
...
create_old_who_db(fromwhotmp) char *fromwhotmp;
{
	int fd;
	int status;
...
create_new_who_db(towhotmp) char *towhotmp;
{
	struct tm *tm;
	time_t now;
	char datebuf[64];
...
copy_link(src, dst, siz) char *src, *dst;
off_t siz;
{
	int cc, status;
...
copy_file(src, dst, fname, fullfname) char *src, *dst;
...
create_new_target(targettmp) char *targettmp;
{
	struct stat statb;
	int status;
...
#ifndef _BLD
install_old_dbs(fromlogtmp, fromwhotmp) char *fromlogtmp, *fromwhotmp;
{
	int status;
...
install_new_dbs(tologtmp, towhotmp) char *tologtmp, *towhotmp;
{
	int status;
...
install_targets(targettmp) char *targettmp;
{
	int i;
	int status;
	char temp[MAXPATHLEN];
...
make_post(stage_p) struct stage *stage_p;
{
	char *ptr;
	char *av[16];
	struct stat statb;
 ...
 */
char *
	buildfname(p, login) register char *p;
char *login;
{
	static char buf[64];
 ...
 */
addheaders(subject, recipients, logfile) char *subject, *recipients, *logfile;
{
	int fd, uid;
	char *up;
	struct passwd *pw;
	...
	if (islower(*progn))
		*progn = toupper(*progn);
	pw = (((up = getenv("USER")) && *up) || ((up = getlogin()) && *up))
			 ? getpwnam(up)
			 : NULL;
...
static _gettemp(as, doopen) char *as;
register int *doopen;
{
