 ...
 * HISTORY
 * $Log: release_db.c,v $
 * Revision 1.8.2.4  1992/12/02  19:39:41  damon
 * 	CR 183. Added CMU notice
 * 	[1992/12/02  19:39:11  damon]
 *
 * Revision 1.8.2.3  1992/07/26  17:37:40  gm
 * 	Removed BSD unix dependencies.
 * 	[1992/07/17  20:52:50  gm]
 *
 * Revision 1.8.2.2  1991/12/30  21:26:17  damon
 * 	port to sparc_sunos and libsb to libode transition
 * 	[1991/12/20  20:13:48  damon]
 *
 * Revision 1.7.1.2  1991/12/17  15:26:30  damon
 * 	Ported to hp300_hpux
 * 	[1991/12/17  14:31:25  damon]
 *
 * Revision 1.7  1991/12/05  20:53:26  devrcs
 * 	Added _FREE_ to copyright marker
 * 	[91/08/01  08:06:35  mckeen]
 *
 * 	rcsid/RCSfile header cleanup
 * 	[90/12/01  18:56:14  dwm]
 *
 * Revision 1.5  90/10/07  21:52:24  devrcs
 * 	Added EndLog Marker.
 * 	[90/09/29  14:06:27  gm]
 *
 * Revision 1.4  90/09/23  16:56:09  devrcs
 * 	Inserted old cmu library version of bsearch as binsearch since
 * 	the osf1 bsearch has a different calling convention.
 * 	[90/09/20  18:04:02  gm]
 *
 * Revision 1.3  90/08/09  14:41:59  devrcs
 * 	Reinstate the release program.
 * 	[90/07/18  09:35:49  gm]
 *
 * Revision 1.2  90/01/02  19:59:20  gm
 * 	Fixes for first snapshot.
 *
 * Revision 1.1  89/12/26  10:47:54  gm
 * 	Current version from CMU.
 * 	[89/12/23            gm]
 *
 * Revision 2.2  89/07/04  17:48:38  gm0w
 * 	Created.
 * 	[89/07/04            gm0w]
 *
 * $EndLog$
...
static char rcsid[] = "@(#)$RCSfile: release_db.c,v $ $Revision: 1.8.2.4 $ (OSF) $Date: 1992/12/02 19:39:41 $";
#endif
#include <sys/param.h>
#include <sys/time.h>
#include <sys/file.h>
...
#define putshort(val, buf) (memcpy(buf, val, sizeof(short)))
 ...
 */
int
	binsearch(base, nel, width, key, compar) char *base; /* start of the array to search */
...
cmpstring(s, len, xhash, str) register struct string *s;
...
struct strhdr *
	prevstring(sdata, slen, ahash, xhash, sptr) register char *sdata;
register int slen;
register int ahash;
register int xhash;
struct string **sptr;
...
linkstring(s, sp) register struct strhdr *s;
...
hashstring(ptr, ahashp, xhashp) register char *ptr;
int *ahashp, *xhashp;
{
...
create_string(ptr, string_ptr_ptr) register char *ptr;
...
cmp_string(string1, string2) struct string *string1, *string2;
{
...
alloc_table(table_ptr, initial_size) register struct table *table_ptr;
{
	if (table_ptr->max == 0)
	{
...
check_dir_name(dir_name, dir_ptr) register struct string *dir_name;
register struct dir **dir_ptr;
{
	return (cmp_string(dir_name, (*dir_ptr)->name));
}

check_file_name(file_name, file_ptr) register struct string *file_name;
register struct r_file **file_ptr;
{
	return (cmp_string(file_name, (*file_ptr)->name));
}

create_dir(dir_name, dir_ptr_ptr) register struct string *dir_name;
...
create_file(file_name, file_ptr_ptr) register struct string *file_name;
...
insert_file(filename, file_ptr_ptr, dir_ptr_ptr, file_info_ptr_ptr) char *filename;
struct r_file **file_ptr_ptr;
struct dir **dir_ptr_ptr;
	...
	for (;;)
	{
		...
		else
		{
			if (dir_ptr->ftable.cur != 0)
			{
				...
				if (i++ != -1 &&
					check_file_name(part_string, (struct r_file **)&dir_ptr->ftable.ftab[i - 1]) == 0)
				{
...
insert_info(owner, group, mode, file_info_ptr_ptr) struct string *owner, *group;
u_short mode;
struct file_info **file_info_ptr_ptr;
{
	struct file_info *fi;
...
read_database_description(db_path) char *db_path;
{
	FILE *fp;
	int lineno;
	int errors;
...
subdirectory_path(dir_ptr, buf) struct dir *dir_ptr;
char *buf;
{
	if (dir_ptr->index != 0)
	{
...
directory_path(dir_ptr, buf) struct dir *dir_ptr;
char *buf;
{
	buf[0] = '\0';
	if (dir_ptr->index != 0)
...
component_path(file_ptr, buf) struct r_file *file_ptr;
char *buf;
{
	buf[0] = '\0';
...
write_database_description(db_path) char *db_path;
{
	FILE *fp;
	int errors;
	char path[BUFSIZ];
...
restore_database(db_path) char *db_path;
{
	char buf[BUFSIZ];
	char *buffer;
	FILE *fp;
...
save_database(db_path) char *db_path;
{
	FILE *fp;
	int errors;
	char buf[BUFSIZ];
...
lookup_component(filename, file_ptr_ptr, file_info_ptr_ptr) char *filename;
struct r_file **file_ptr_ptr;
struct file_info **file_info_ptr_ptr;
{
	char file[BUFSIZ];
...
create_component(file, file_ptr_ptr, file_info_ptr_ptr) char *file;
struct r_file **file_ptr_ptr;
struct file_info **file_info_ptr_ptr;
{
...
set_component_info(file_ptr, file_info_ptr_ptr, srcdir, nlinks, links,
				   owner, group, mode) struct r_file *file_ptr;
struct file_info **file_info_ptr_ptr;
char *srcdir;
int nlinks;
char **links;
...
lock_database(db_path) char *db_path;
{
	char buf[MAXPATHLEN];
	char *user;
	char *tty;
	...
	errno = 0;
	if (lseek(lock_fd, (long)0, SEEK_SET) == -1 && errno != 0)
		ewarn("lseek");
