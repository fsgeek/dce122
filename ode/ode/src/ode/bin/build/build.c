...
void cmdline_syntax(char **sb_rcfile, char **basepath)
...

{
  ...
  *sb_rcfile = NULL;
  ...
  else
  {
    ...
    if ((targetdirs = (char **)malloc((unsigned)ntargets * sizeof(char *))) == (char **)NULL)
      uquit(ERROR, FALSE, "targetdirs malloc failed\n");
  } /* else */
} /* cmdline syntax */

void resize_makeargs(int *maxmakeargs)
{
  if (*maxmakeargs == 0)
  {
    *maxmakeargs = 16;
...
void check_sandbox(struct rcfile *rcfile_p)
{
  ...
  if (get_rc_value("source_owner", &rcfile_source_owner, rcfile_p, FALSE) != OK)
    rcfile_source_cover = NULL;

  else
  {

    if (get_rc_value("source_cover", &rcfile_source_cover, rcfile_p, TRUE) != OK)
  ...
  if (get_rc_value("object_base", &rcfile_object_base, rcfile_p, TRUE) != OK)
  ...
  if (get_rc_value("object_owner", &rcfile_object_owner, rcfile_p, FALSE) != OK)
  {
  ...
  else
  {
    ...
    if (get_rc_value("object_cover", &rcfile_object_cover, rcfile_p, TRUE) != OK)
...
void print_envvar(struct arg_list *var)
{
  char *val;
...
void build_targets(void)
{
  ...
  for (j = 0; j < ntargets; j++)
  {
    ...
    if (!ui_is_info())
    { /* make runcmd conditional on -info */
